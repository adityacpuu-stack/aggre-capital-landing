...
export default function DashboardPage() {
  ...
  const fetchAllApplications = async () => {
    try {
      ...
      console.log(
        ...
        applications
          .slice(0, 5)
          .map((app: any) => ({ id: app.application_id, status: app.status })),
      );
  ...
  const handleSaveTestimonial = async (
    ...
    try {
      const method = isEdit ? "PUT" : "POST";
      const url = isEdit
        ? `/api/testimonials/${testimonialData.id}`
        : "/api/testimonials";
  ...
  const handleSavePartner = async (partnerData: any, isEdit: boolean) => {
    try {
      const method = isEdit ? "PUT" : "POST";
      const url = isEdit
        ? `/api/partners/strategic/${partnerData.id}`
        : "/api/partners/strategic";
  ...
  const handleSaveEcosystemPartner = async (
    ...
    try {
      const method = isEdit ? "PUT" : "POST";
      const url = isEdit
        ? `/api/partners/ecosystem/${partnerData.id}`
