...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      ...
      paramCount++;
      const contentParam = paramCount;
      paramCount++;
      const excerptParam = paramCount;
      whereClause += ` AND (title ILIKE $${titleParam} OR content ILIKE $${contentParam} OR excerpt ILIKE $${excerptParam})`;
